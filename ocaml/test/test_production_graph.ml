...
let () =
  ...
  (* An unknown dependency never reaches the graph: Specification.parse's own
     validate pass rejects it with Unknown_dependency, and build only accepts a
     Specification.t. So Production_graph.Unknown_requirement is unreachable
     through the public API, and asserting it here required input that cannot
     parse -- which is why this file failed before the branches were merged.
     The reachable contract is asserted instead. *)
  begin match Specification.parse "req a : first\ndepends a : missing\n" with
  | Error (Specification.Unknown_dependency { id = "a"; dependency = "missing" }) ->
      incr pass
  | _ -> check "an unknown dependency is rejected at parse time" false
  end;
